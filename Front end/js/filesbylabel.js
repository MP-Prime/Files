...
let label = urlPmtr.get('label');

let regex = /^[a-z0-9]+$/i;
let check = regex.test(label);

// * Testing for label and regex
/* console.log(label);
console.log(`The labels of the item I want is ${label}`);
console.log(check); */

if(check === true){

    // * Fetch all by label
    fetch(`http://localhost:8091/livefiles/read/label/${label}`)
    .then((response) => {
